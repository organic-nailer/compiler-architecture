...
class SLR1Parser(
    private val transitionMap: Map<Pair<String, String>, SLR1ParserGenerator.TransitionData>
) {
    fun parse(input: List<String>): Node? {
        ...
        while(parseIndex < input.size || stack.isNotEmpty()) {
            ...
            when(transition?.kind) {
                SLR1ParserGenerator.TransitionKind.SHIFT -> {
                ...
                SLR1ParserGenerator.TransitionKind.REDUCE -> {
                ...
                SLR1ParserGenerator.TransitionKind.ACCEPT -> {
                    accepted = true
                    break
                }
                else -> {
